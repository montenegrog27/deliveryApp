...
if (type === "button") {
  ...
  if (!originalMessageId || !phone) {
    console.warn("⚠️ No hay context.id o teléfono");
    return new Response("Faltan datos", { status: 200 });
  }

  // Buscar trackingId por messageId en whatsapp_chats
  const chatRef = doc(db, "whatsapp_chats", phone);
  const chatSnap = await getDoc(chatRef);

  if (!chatSnap.exists()) {
    console.warn("⚠️ Chat no encontrado:", phone);
  ...
  if (payload?.toLowerCase().includes("confirmar")) {
    if (order.status === "cancelled") {
      baseMessage.message = "⚠️ Intentó confirmar un pedido cancelado.";
      await sendText(phone, "⚠️ El pedido ya fue cancelado.");
    } else if (order.clientConfirm === true) {
      baseMessage.message = "⚠️ Pedido ya confirmado.";
      await sendText(phone, "⚠️ Ya confirmaste tu pedido.");
    } else {
      await updateDoc(orderRef, {
        clientConfirm: true,
        clientConfirmAt: serverTimestamp(),
      ...
      baseMessage.message = "✅ Pedido confirmado por el cliente.";
      await sendText(phone, "✅ Pedido confirmado. ¡Gracias!");
    }
  }

  if (payload?.toLowerCase().includes("cancelar")) {
    if (order.clientConfirm === true) {
      baseMessage.message = "⚠️ Intentó cancelar un pedido confirmado.";
      await sendText(phone, "⚠️ El pedido ya fue confirmado y no puede cancelarse.");
    } else if (order.status === "cancelled") {
      baseMessage.message = "⚠️ Pedido ya cancelado.";
      await sendText(phone, "⚠️ Ya cancelamos tu pedido.");
    } else {
      await updateDoc(orderRef, {
        clientConfirm: false,
        clientCancelAt: serverTimestamp(),
      ...
      baseMessage.message = "❌ Pedido cancelado por el cliente.";
      await sendText(phone, "❌ Pedido cancelado. Podés hacer otro en cualquier momento.");
    }
  }
